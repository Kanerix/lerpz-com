...
export function useChat(options: UseChatOptions = {}) {
    ...
    // Keep callback refs up-to-date so `send` never captures stale closures
    // while still keeping a stable identity (no dependency churn).
                    ...
                    onMessage: (data, event) => {
                        switch (event) {
                            ...
                            case "saved": {
                                ...
                                onSavedRef.current?.(conversationId);
                                break;
                            }
                    ...
                    onClose: (incomplete) => {
                        if (incomplete) {
                            setState((prev) => ({
                                ...
                                isStreaming: false,
                                error:
                                    prev.error ?? "Stream ended unexpectedly.",
                            }));
                        }
                        closeRef.current = null;
                    },
