...
/**
 * Creates a new conversation and streams the AI reply via Server-Sent Events. Events: `conversation_created` (new conversation UUID, sent first), `reasoning` (chain-of-thought chunk, reasoning models only), `message` (answer token chunk), `saved` (conversation UUID confirming persistence, sent last), `error` (problem document describing the failure).
